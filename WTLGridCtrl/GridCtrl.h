...
//
// - Changed CRuntimeClass to ICellTypeFactory 
// - Changed MFC Printing stuff to WTL CPrintPreview (VC 6) and
//	 CZoomPrintPreview
// - Changed MFC COleDataSource/COleDataObject to IDataObject and CDropTargetImpl
// - Clipboard support by Bjarke Viksoe in "atldataobj.h"
// - Added a new class CSystemCursor because of a bug in WTL CCursorT
//   LoadOEMCursor needs to be defined as LoadOEMCursor(ATL::_U_STRINGorID)
...
#ifndef GRIDCONTROL_NO_PRINTING
#   include <atlprint.h>
#endif
...
// Construction
public:
	DECLARE_WND_CLASS_EX(GRIDCTRL_CLASSNAME,
						 CS_DBLCLKS | CS_HREDRAW | CS_VREDRAW,
						 COLOR_3DFACE);

	typedef CWindowImpl< CGridCtrl, ATL::CWindow, CControlWinTraits >  __baseClass;
	...
	BEGIN_MSG_MAP(CGridCtrl)
		...
		MESSAGE_HANDLER_EX(WM_GETFONT, OnGetFont)
		MESSAGE_HANDLER_EX(WM_IME_CHAR, OnImeChar)
		NOTIFY_HANDLER_EX(IDC_INPLACE_CONTROL, GVN_ENDLABELEDIT, OnEndInPlaceEdit)

#ifndef GRIDCONTROL_NO_CLIPBOARD
		CHAIN_MSG_MAP(CUpdateUI<CGridCtrl>)
#endif
		REFLECT_NOTIFICATIONS()
	END_MSG_MAP()

	HWND Create(HWND hWndParent, const CRect & rect, UINT nID,
...
public:
    ...
    int GetSelectedCount() const                  { return (int)m_SelectedCellMap.size(); }
...
public:
    ...
    BOOL SetCellType(int nRow, int nCol, ICellTypeFactory* pRuntimeClass);
    BOOL SetDefaultCellType( ICellTypeFactory* pRuntimeClass);
...
///////////////////////////////////////////////////////////////////////////////////
    ...
    BOOL CopyTextFromGrid(CString & sSelection);
    BOOL PasteTextToGrid(CCellID cell, IDataObject* pDataObject, BOOL bSelectPastedCells=TRUE);
#endif
 ...
 public:
    ...
    void OnDragLeave();
    BOOL OnDrop(IDataObject* pDataObject, DWORD dwKeyState, DROPEFFECT * pDropEffect, CPoint point);
#endif

#ifndef GRIDCONTROL_NO_CLIPBOARD
	LRESULT OnEditCut(WORD wNotifyCode = 0, WORD wID = 0, HWND hWndCtl = NULL);
	LRESULT OnEditCopy(WORD wNotifyCode = 0, WORD wID = 0, HWND hWndCtl = NULL);
	LRESULT OnEditPaste(WORD wNotifyCode = 0, WORD wID = 0, HWND hWndCtl = NULL);
#endif
...
public:
	...
	int GetNumPages() const { return m_nNumPages; }
	void Print();
	void Print(CPrinter & printer, CDevMode & devmode);
...
public:
	...
	{
		return (nPage < UINT(m_nNumPages) && nPage >= 0);
	}
...
protected:
	...
	virtual void PrintFixedRowCells(int nStartColumn, int nStopColumn, int& row, CRect& rect,
                                    CDC *pDC, BOOL& bFirst);
    virtual void PrintColumnHeadings(CDC *pDC);
    virtual void PrintHeader(CDC *pDC, LPCRECT pRect);
    virtual void PrintFooter(CDC *pDC, LPCRECT pRect, int nCurrPage);
    virtual void PrintRowButtons(CDC *pDC);
#endif
...
protected:
	...
	std::vector<UINT> m_arRowHeights, m_arColWidths;
    //CUIntArray  m_arRowHeights, m_arColWidths;
    int         m_nVScrollMax, m_nHScrollMax;

    // Fonts and images
	ICellTypeFactory*   m_pRtcDefault; // determines kind of Grid Cell created by default
	...
	selMap m_SelectedCellMap;
	selMap m_PrevSelectedCellMap;
...
#ifndef GRIDCONTROL_NO_DRAGDROP
    CDropTarget <CGridCtrl> m_DropTarget;       // OLE Drop target for the grid
#endif
	...
	int         l_nHeaderHeight, l_nFooterHeight, l_nLeftMargin,
				l_nRightMargin, l_nTopMargin, l_nBottomMargin, l_nGap;
...
//////////////////////////////////////////////////////////////////////////
//
// This class is to fix a bug in CCursorT for LoadOEMCursor
// IDC_ cursor are defined as MAKEINTRESOURCE strings
//
class CSystemCursor : public CCursorT<false>
{
public:
	HCURSOR LoadOEMCursor(ATL::_U_STRINGorID cursor)
	{
		ATLASSERT(m_hCursor == NULL);
		m_hCursor = ::LoadCursor(NULL, cursor.m_lpstr);
		return m_hCursor;
	}
};
