...
// Mere translation: changed MFC GDI Objects to WTL
//					 IMPLEMENT_DYNAMIC deleted
...
BOOL CGridCellBase::Draw(CDC* pDC, int nRow, int nCol, CRect rect,  BOOL bEraseBkgnd /*=TRUE*/)
{
    ...
    if (IsFixed() && pGrid->GetGridLines() != GVL_NONE)
    {
        ...
        else
        {
            ...
            CPenHandle oldPen = pDC->GetCurrentPen();

            pDC->SelectPen(lightpen);
            pDC->MoveTo(rect.right, rect.top);
            pDC->LineTo(rect.left, rect.top);
            pDC->LineTo(rect.left, rect.bottom);

            pDC->SelectPen(darkpen);
            pDC->MoveTo(rect.right, rect.top);
	...
	if (!pGrid->m_bPrinting)
#endif
    {
		pDC->SelectFont(GetFontObject()->m_hFont);
    }
        ...
        if (pGrid->GetImageList()->GetImageInfo(GetImage(), &Info))
        {
            ...
            */
            ...
            if ((nImageWidth + rect.left <= rect.right) && (nImageHeight + rect.top <= rect.bottom))                
                pGrid->GetImageList()->Draw(pDC->m_hDC, GetImage(), rect.TopLeft(), ILD_NORMAL);
    ...
    if (pGrid->GetSortColumn() == nCol && nRow == 0)
    {
        ...
        if (size.cx + rect.left < rect.right)
        {
            ...
            if (pGrid->GetSortAscending())
            {
                ...
                CPenHandle oldPen = pDC->SelectPen(penLight);
...
CSize CGridCellBase::GetTextExtent(LPCTSTR szText, CDC* pDC /*= NULL*/)
{
    ...
    CFontHandle oldFont;
    oldFont = pDC->SelectFont(GetFontObject()->m_hFont);
    ...
    if ((nFormat & DT_WORDBREAK) && !(nFormat & DT_SINGLELINE))
    {
        ...
        while (TRUE)
        {
			...
			CSize s;
			pDC->GetTextExtent(TempStr, TempStr.GetLength(), &s);
			int nTempWidth = s.cx;
        ...
        pDC->GetTextExtent(szText, _tcslen(szText), &size);
