...
BOOL CGridCell::Edit(int nRow, int nCol, CRect rect, CPoint /* point */, UINT nID, UINT nChar)
{
	...
	{   
		...
		m_pEditWnd = new CInPlaceEdit();
		m_pEditWnd->Create(*pGrid, rect, dwStyle, nID, nRow, nCol, GetText(), nChar);
    }
    return TRUE;
}
...
void CGridDefaultCell::SetFont(const LOGFONT* plf)
{
    ...
    if (!dc.IsNull())
    {
		CFontHandle oldFont = dc.SelectFont(m_Font.m_hFont);
		CSize sz;
		dc.GetTextExtent(_T(" "), 1, &sz);

        SetMargin(sz.cx);
        
		if (dc.GetTextExtent(_T(" XXXXXXXXXXXX "), 14, &m_Size))
