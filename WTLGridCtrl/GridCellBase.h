...
// Mere translation: ATLASSERT for ASSERT
//					 CWindow for CWnd
//					 DECLARE_DYNCREATE deleted
//
// Added ICellTypeFactory and CCellTypeFactoryImpl to substitute MFC
// RUNTIME_CLASS and CRuntimeClass
...
interface ICellTypeFactory
{
	virtual CGridCellBase* CreateObject() = 0;
};

template < class T >
class CCellTypeFactoryImpl : public ICellTypeFactory
{
	virtual CGridCellBase* CreateObject()
	{
		T* p = new T;
		return dynamic_cast<CGridCellBase*>(p);
	}
};


#endif //__GRIDCELLBASE_H__
